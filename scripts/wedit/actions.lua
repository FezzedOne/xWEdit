...
function wedit.actions.WE_AllInOne()
    if not status.statusProperty("wedit.compact.open") then
        ...
        if not c.fireLocked and (c.primaryFire or c.altFire) then
            c.fireLock()
            if player then
                player.interact("ScriptPane", "/interface/wedit/compact/compact.config")
            else
                world.sendEntityMessage(
                    entity.id(),
                    "interact",
                    "ScriptPane",
                    "/interface/wedit/compact/compact.config"
                )
            end
        end
    end
...
function wedit.actions.WE_Select()
    ...
    if controller.validSelection() then
        ...
        local w, h =
            controller.selection[2][1] - controller.selection[1][1],
            controller.selection[2][2] - controller.selection[1][2]
    ...
    elseif controller.selectStage == 1 then
        ...
        if controller.primaryFire then
            ...
            controller.selection[1][1] = math.floor(
                (controller.rawSelection[1][1] < controller.rawSelection[2][1]) and controller.rawSelection[1][1]
                    or controller.rawSelection[2][1]
            )
            controller.selection[2][1] = math.ceil(
                (controller.rawSelection[1][1] < controller.rawSelection[2][1]) and controller.rawSelection[2][1]
                    or controller.rawSelection[1][1]
            )

            -- Compare Y (1 is smallest):
            controller.selection[1][2] = math.floor(
                (controller.rawSelection[1][2] < controller.rawSelection[2][2]) and controller.rawSelection[1][2]
                    or controller.rawSelection[2][2]
            )
            controller.selection[2][2] = math.ceil(
                (controller.rawSelection[1][2] < controller.rawSelection[2][2]) and controller.rawSelection[2][2]
                    or controller.rawSelection[1][2]
            )
        else
...
function wedit.actions.WE_Undo()
    ...
    if backupSize > 0 then
        ...
        if controller.validSelection() and math.ceil(controller.selection[2][2]) == math.ceil(top) then
            top = top + 1
        end
        wedit.debugRenderer:drawText("^shadow;WEdit Undo Position", { backup.origin[1], top }, "#FFBF87")
        wedit.debugRenderer:drawRectangle(
            backup.origin,
            { backup.origin[1] + backup.size[1], backup.origin[2] + backup.size[2] },
            "#FFBF87"
        )
    end
    ...
    if not controller.fireLocked then
        if controller.primaryFire then
            -- Undo
            controller.fireLock()
            if backupSize > 0 then wedit.paste(controller.backup[backupSize], controller.backup[backupSize].origin) end
        elseif controller.altFire then
            -- Remove Undo
            controller.fireLock()
            if backupSize > 0 then table.remove(controller.backup, backupSize) end
        end
    end
end
...
function wedit.actions.WE_ColorPicker()
    ...
    controller.info(
        "^shadow;^yellow;Current Block: ^red;" .. controller.selectedBlockToString() .. "^yellow;.",
        { 0, -5 }
    )
    ...
    if controller.shiftHeld then
        if not controller.shiftFireLocked and (controller.primaryFire or controller.altFire) then
            ...
            if player then
                player.interact("ScriptPane", materialPickerLoader.config)
            else
    ...
    if not controller.fireLocked and controller.validSelection() then
        if (not controller.shiftHeld) or not overgroundSupport then
            if controller.primaryFire then
                ...
                local backup = wedit.fillBlocks(
                    controller.selection[1],
                    controller.selection[2],
                    "foreground" .. curOver.action,
...
function wedit.actions.WE_Pencil()
    ...
    controller.info(
        "^shadow;^yellow;Primary Fire: Draw on ^orange;foreground " .. curOver.name .. "^yellow;.",
        { 0, -1 }
    )
    ...
    local debugCallback = function(pos) wedit.debugRenderer:drawBlock(pos) end

    local layer = controller.primaryFire and ("foreground" .. curOver.action)
        or controller.altFire and ("background" .. curOver.action)
        or nil
...
function wedit.actions.WE_BlockPinner()
    ...
    local fgh, bgh =
        world.materialHueShift(aimPos, "foreground") or 0, world.materialHueShift(aimPos, "background") or 0
    if fg then
            ...
            if type(block) ~= "boolean" then
                ...
                if not tileCfg then
                    ...
                    if not tileCfg.config.itemDrop then
                        ...
                        icon = controller.fixImagePath(itemCfg.directory, itemCfg.config.inventoryIcon)
                            .. "?hueshift="
                            .. math.floor(hueshift * 360 / 255)
                        sb.logInfo("Block hueshift: %s", hueshift)
                        sb.logInfo("Icon: %s", icon)
                        local params = controller.spawnOreParameters(
                            "WE_Block",
...
function wedit.actions.WE_Block()
    ...
    controller.info(
        "^shadow;^yellow;Primary Fire: Place in ^orange;foreground " .. curOver.name .. "^yellow;.",
        { 0, -1 }
    )
    ...
    if itemData and itemData.block then
        controller.info(
            "^shadow;^yellow;Material: ^red;" .. itemData.block .. "^yellow;.",
            { 0, overgroundSupport and -4 or -3 }
        )
    else
...
function wedit.actions.WE_Stamp()
    ...
    if controller.validSelection() then controller.showSelection() end
...
function wedit.actions.WE_Flip()
    ...
    if not controller.fireLocked and controller.primaryFire then
        controller.fireLock()
        if c then storage.weditCopy = wedit.flip(storage.weditCopy, "horizontal") end
    elseif not controller.fireLocked and controller.altFire then
        controller.fireLock()
        if c then storage.weditCopy = wedit.flip(storage.weditCopy, "vertical") end
    end
end
...
function wedit.actions.WE_SchematicMaker()
    ...
    if not controller.fireLocked and controller.primaryFire and controller.validSelection() then
        ...
        local params = controller.spawnOreParameters(
            "WE_Schematic",
            "^yellow;Primary Fire: Paste Schematic.",
            "^orange;WEdit: Schematic " .. schematicID,
            icon,
            "essential"
        )
...
function wedit.actions.WE_Schematic()
    ...
    if controller.validSelection() and schematicID and schematic then
        ...
        wedit.debugRenderer:drawRectangle(
            controller.selection[1],
            { controller.selection[1][1] + schematic.size[1], top },
            "cyan"
        )
...
function wedit.actions.WE_Replace()
    local fgTile, bgTile =
        world.material(tech.aimPosition(), "foreground"), world.material(tech.aimPosition(), "background")
    local overgroundSupport = not not world.objectDirection
    ...
    controller.info(
        "^shadow;^yellow;Replace Block: ^red;"
            .. controller.blockToString(fgTile)
            .. "^yellow; / ^red;"
            .. controller.blockToString(bgTile),
        { 0, -4 }
    )
    controller.info(
        "^shadow;^yellow;Replace With: ^red;" .. controller.selectedBlockToString() .. "^yellow;.",
        { 0, -5 }
    )
    if overgroundSupport then
    ...
    if not controller.shiftFireLocked and controller.validSelection() then
        ...
        if layer then
            controller.shiftFireLock()
            local backup = wedit.replace(
                controller.selection[1],
                controller.selection[2],
                layer,
                controller.selectedBlock,
                not controller.shiftHeld and tile
            )
            if backup then table.insert(controller.backup, backup) end
        end
    end
...
function wedit.actions.WE_Modifier()
    ...
    if controller.shiftHeld then
        if not controller.shiftFireLocked and (controller.primaryFire or controller.altFire) then
            ...
            if player then
                player.interact("ScriptPane", matmodPickerLoader.config)
            else
...
function wedit.actions.WE_ModPinner()
    ...
    if not controller.fireLocked then
        if controller.primaryFire or controller.altFire then
            ...
            local params = controller.spawnOreParameters(
                "WE_Mod",
                "^yellow;Primary Fire: Modify foreground.\nAlt Fire: Modify background.",
                "^orange;WEdit: " .. mod .. " MatMod",
                icon,
                "essential"
            )
...
function wedit.actions.WE_Ruler()
    ...
    if overgroundSupport then
        controller.info(
            "^shadow;^yellow;Use a pinned block or the pencil or fill tool to switch collision placement mode.",
            { 0, -6 }
        )
    end
    ...
    -- Draw line
    if
        not wedit.ruler.selecting
        and controller.shiftHeld
        and controller.primaryFire
        and not controller.shiftFireLocked
    then
        controller.shiftFireLock()
    ...
    if not controller.shiftFireLocked and not wedit.ruler.selecting then
        ...
        elseif not controller.shiftHeld then
            -- Fill line
            local layer = controller.primaryFire and ("foreground" .. curOver.action)
                or controller.altFire and "background"
                or nil
            if layer and controller.validLine() then
                ...
                wedit.line(
                    line[1],
                    line[2],
                    controller.primaryFire and layer or "background",
                    controller.selectedBlockToString()
                )
            end
        end
    end
    ...
    if controller.validLine() then
        ...
        wedit.bresenham(line[1], line[2], function(x, y)
        ...
        controller.info(
            "^shadow;^yellow;Current Length: ^red;"
                .. length
                .. " ^yellow;blocks ^red;("
                .. w
                .. "x"
                .. h
                .. ")^yellow;.",
            { 0, overgroundSupport and -7 or -6 }
        )
    end
end
...
function wedit.actions.WE_Hydrator()
    ...
    if not controller.fireLocked then
        ...
        elseif controller.altFire then
            ...
            if player then
                player.interact("ScriptPane", liquidPickerLoader.config)
            else
...
function wedit.actions.WE_Dye()
    ...
    if controller.shiftHeld then
        if not controller.shiftFireLocked and (controller.primaryFire or controller.altFire) then
            if player then
                player.interact("ScriptPane", "/interface/wedit/dyePicker/dyePicker.config")
            else
                world.sendEntityMessage(
                    entity.id(),
                    "interact",
                    "ScriptPane",
                    "/interface/wedit/dyePicker/dyePicker.config"
                )
            end
            controller.shiftFireLock()
        end
    elseif not controller.shiftFireLocked then
        local callback = function(pos)
            ...
            if layer then wedit.dye(pos, layer, colorIndex) end
        end
...
function wedit.actions.WE_Hue()
    ...
    if controller.shiftHeld then
        if not controller.shiftFireLocked and controller.altFire then
            if player then
                player.interact("ScriptPane", "/interface/wedit/huePicker/huePicker.config")
            else
                world.sendEntityMessage(
                    entity.id(),
                    "interact",
                    "ScriptPane",
                    "/interface/wedit/huePicker/huePicker.config"
                )
            end
            controller.shiftFireLock()
        elseif controller.primaryFire then
    ...
    elseif not controller.shiftFireLocked then
        local callback = function(pos)
            ...
            if layer then
                ...
                if block and hue ~= blockHue then wedit.pencil(pos, layer, block, hue, true) end
            end
        end
...
function wedit.actions.WE_RandomFill()
    ...
    if controller.shiftHeld then
        if not controller.shiftFireLocked and (controller.primaryFire or controller.altFire) then
            if player then
                player.interact("ScriptPane", "/interface/wedit/randomPicker/randomPicker.config")
            else
                world.sendEntityMessage(
                    entity.id(),
                    "interact",
                    "ScriptPane",
                    "/interface/wedit/randomPicker/randomPicker.config"
                )
            end
            controller.shiftFireLock()
        end
...
function wedit.actions.WE_RandomPencil()
    ...
    if overgroundSupport then
        controller.info(
            "^shadow;^yellow;Use a pinned block or the pencil or fill tool to switch overground placement mode.",
            { 0, -4 }
        )
        controller.info("^shadow;^yellow;Current percentage: ^red;" .. perc .. "^yellow;.", { 0, -5 })
        controller.info(
    ...
    local layer = controller.primaryFire and curOver.action or controller.altFire and "background" or nil
    ...
    elseif not controller.shiftFireLocked then
        if layer then controller.shiftFireLock() end
